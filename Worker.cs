			...
			try
            {
                using (var scope = _scopeFactory.CreateScope())
                {
					...
					var tblProjects = _OCOCContext.TblProjects
                        .Where(proj => proj.SyncStatus == 1 || proj.SyncStatus == 2)
                        .AsNoTracking()
                        .ToList();
			...
			if (tblAddenda != null && tblAddenda.Count > 0)
			{
				foreach (TblAddendum adden in tblAddenda)
				{
					try
					{
						...
						// Set SyncStatus to 3 (processed) after successful save
						adden.SyncStatus = 3;
				...
				try
				{
					...
					foreach (TblProjCounty pc in lstpc)
					{
						try
						{
							...
							_logger.LogInformation($"Starting sync for ProjCounty ID {pc.CountyId}.");

							if (pc.SyncStatus == 1)
							{
								var county = _PCNWContext.Counties.FirstOrDefault(c => c.CountyId == pc.CountyId);
								if (county != null)
								{
									...
									_PCNWContext.SaveChanges();
									_logger.LogInformation($"ProjCounty ID {pc.CountyId} added to ProjCounty table.");
									SuccessProjCountyProcess++;
								}
								else
								{
									_logger.LogWarning($"County not found for ProjCounty ID {pc.CountyId}.");
							...
							{
								projCounty = _PCNWContext.ProjCounties.FirstOrDefault(proCou => proCou.SyncProCouId == pc.ProjCountyId);
								if (projCounty != null)
								{
									projCounty.CountyId = pc.CountyId;
									_PCNWContext.Entry(projCounty).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
									_PCNWContext.SaveChanges();
									_logger.LogInformation($"ProjCounty ID {pc.CountyId} updated successfully.");
									SuccessProjCountyProcess++;
								}
								else
								{
									_logger.LogWarning($"ProjCounty ID {pc.CountyId} not found for update.");
									FailProjCountyProcess++;
								}
							}
							...
							_OCOCContext.SaveChanges();
							_logger.LogInformation($"Sync status updated to 3 for ProjCounty ID {pc.CountyId}.");
						}
						catch (Exception exProjCounty)
						{
							_logger.LogError($"Exception occurred for ProjCounty ID {pc.CountyId}.", exProjCounty);
							FailProjCountyProcess++;
							continue;
						}
					}

					// Handle additional PreBidInfo and EstCost logic here...
